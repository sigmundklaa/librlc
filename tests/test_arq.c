...
static rlc_errno tx_submit(struct rlc_context *rlc, gnb_h buf)
{
...
static void test_arq(void)
{
        ...
        buf = gnb_new(&c1->rlc.alloc_gnb, sizeof(TEST_DATA));
        TEST_ASSERT_TRUE(gnb_okay(buf));

        gnb_put(&buf, (uint8_t *)TEST_DATA, sizeof(TEST_DATA));
